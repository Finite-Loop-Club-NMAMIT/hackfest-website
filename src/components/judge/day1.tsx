...
export default function DAY1() {
  ...
  // Replace separate search states with a single search query
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [selectedTrack, setSelectedTrack] = useState<string>("all");
  
  // Update filtered teams to use single search query
  const filteredTeams = useMemo(() => {
    if (!teamsQuery.data) return [];
    
    let filtered = teamsQuery.data;
    ...
    return filtered;
  }, [teamsQuery.data, selectedTrack, searchQuery]);
  ...
  const handleSaveRemarks = useCallback(async (indexToSave: number) => {
    if (isSaving || !filteredTeams || !filteredTeams[indexToSave] || !session?.user) return;
    const remarksToSave = currentRemarks.filter((point: string) => point.trim() !== '');
    const existingSavedRemarkObj = filteredTeams[indexToSave]?.Remark?.find(r => r.judgeId === session.user.id);
    ...
    setIsSaving(true);
    const teamId = filteredTeams[indexToSave].id;
    ...
    try {
        ...
        toast.success(`Remarks for Team ${filteredTeams[indexToSave].teamNo} auto-saved`);
    } catch (error) {
        toast.error("Auto-save failed:");
    }
  }, [addRemarkMutation, currentRemarks, isSaving, session?.user, filteredTeams]);

  // Auto-scroll to first matching team when search query changes
  useEffect(() => {
    if (carouselApi && filteredTeams.length > 0 && searchQuery.trim() !== "") {
      // Find the first team that matches the search query
  ...
  }, [searchQuery, filteredTeams, carouselApi]);
  ...
  useEffect(() => {
    if (carouselApi && filteredTeams && filteredTeams.length > currentTeamIndex) {
      carouselApi.scrollTo(currentTeamIndex, true);
    }
  }, [carouselApi, filteredTeams, currentTeamIndex]);

  useEffect(() => {
    if (filteredTeams && filteredTeams[currentTeamIndex] && session?.user && currentRemarks.length === 0) {
      const userRemark = filteredTeams[currentTeamIndex]?.Remark?.find(r => r.judgeId === session.user.id);
  ...
  }, [filteredTeams, currentTeamIndex, currentRemarks.length, session?.user]);
  ...
  useEffect(() => {
    ...
    const handleSelect = () => {
        void (async () => {
            ...
            if (filteredTeams && filteredTeams[newIndex]) {
                const userRemark = filteredTeams[newIndex]?.Remark?.find(r => r.judgeId === session.user.id);
                const existingRemark = userRemark?.remark;
  ...
  }, [carouselApi, filteredTeams, handleSaveRemarks, currentTeamIndex, session?.user]);
  ...
  // Update tutorial steps to include search filters
  const day1TutorialSteps: Step[] = [
    ...
    },
    {
      ...
      content: 'Use these controls to search for teams or filter by track. The search happens automatically as you type.',
      placement: 'bottom',
    },
    {
  ...
  return (
    <>
      ...
      {teamsQuery.isSuccess && (
        <div className="w-full px-2 py-4 md:px-6 lg:px-8 md:py-6 lg:py-8 bg-background">
          {/* Modified search and filter controls */}
          <div className="filter-search-controls mb-4 flex w-full max-w-5xl mx-auto flex-col items-center gap-3 px-2 md:flex-row md:gap-4">
            <div className="flex w-full flex-col gap-1 md:w-1/2">
              <Label htmlFor="search-team" className="text-xs font-medium text-muted-foreground">Search by Team Name or Number</Label>
          ...
          <div className="flex justify-center mb-2">
            <p className="text-sm text-muted-foreground">
              Showing {filteredTeams.length} of {teamsQuery.data?.length || 0} teams
            </p>
          </div>
          
          {/* Carousel component */}
          {filteredTeams.length > 0 ? (
            <div className="carousel-container flex w-full items-center justify-center overflow-hidden bg-background">
              <Carousel
              ...
              >
                <CarouselContent className="overflow-visible">
                  {filteredTeams.map((team: TeamWithRelations, index: number) => {
            ...
            <div className="flex h-64 w-full flex-col items-center justify-center text-center p-4">
              <p className="text-lg text-muted-foreground mb-2">No teams match your search criteria.</p>
              <Button 
                variant="outline" 
                onClick={() => {
                  setSearchQuery("");
      ...
      {teamsQuery.isSuccess && (!teamsQuery.data || teamsQuery.data.length === 0) && (
